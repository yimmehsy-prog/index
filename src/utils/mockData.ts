...
export const getDramaRankings = (currency: Currency, languageFilter: string = 'ALL') => {
  ...
  const generateDramaMetrics = (revenue: number) => {
    const roi = +(Math.random() * 0.6 + 0.9).toFixed(2); // 0.9 to 1.5
    const spend = Math.round(revenue / roi);
    ...
    const spendTrend = Math.round(Math.random() * 40 - 15); // -15% to +25%
    
    return { spend, spendTrend, roi, netRevenue, viewingUsers, intentionCount, payingUsers };
  };
...
export const getChannelData = (currency: Currency, date: 'today' | 'yesterday'): ChannelData[] => {
  const rate = currency === 'CNY' ? 7.2 : 1;
  const m = date === 'today' ? 1 : 1.15; // multiplier for yesterday
  ...
  const data: ChannelData[] = [
    ...
    { 
      ...
      spend: 50000 * rate * m, roi: 1.60, 
      dramaName: 'Billionaire\'s Secret Wife', dramaId: '1001', language: '英语',
      department: '市场部二组'
    },
    { 
      id: 'CH005', name: '金大卫', leadName: '王磊', platform: 'fb', type: 'self', 
      recharge: 60000 * rate * m, rechargeUsers: 480, 
      newRecharge: 15000 * rate * m, newRechargeUsers: 120,
      spend: 55000 * rate * m, roi: 1.09, 
      dramaName: 'Alpha\'s Rejected Mate', dramaId: '1002', language: '英语',
      department: '市场部三组'
    },
    { 
    ...
    { 
      id: 'CH008', name: '短剧推广公司', leadName: '周杰', platform: 'fb', type: 'dist', 
    ...
    { 
      id: 'CH009', name: '亚洲触达', leadName: '吴刚', platform: 'tk', type: 'dist', 
    ...
    { 
      id: 'CH010', name: '欧洲流媒体', leadName: '郑爽', platform: 'fb', type: 'dist', 
