...
import { 
  Globe, Activity, DollarSign, Users, Smartphone, Film, TrendingUp, Award, Clock, Filter, Info, CreditCard, Map, HelpCircle, X, BookOpen, Link
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const leadRankings = React.useMemo(() => {
    const leads: Record<string, { name: string, recharge: number, spend: number, roi: number, count: number, department: string }> = {};
    
    rawChannelData.forEach(item => {
      if (item.leadName && item.leadName !== '-') {
        if (!leads[item.leadName]) {
          leads[item.leadName] = { name: item.leadName, recharge: 0, spend: 0, roi: 0, count: 0, department: item.department };
        }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-12">
      ...
      <div className="bg-white border-b border-slate-200 sticky top-16 z-20 shadow-sm">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between overflow-x-auto">
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2 pt-2 border-t border-slate-100">
                          <div>
                            <div className="text-[9px] text-slate-400 font-medium uppercase tracking-wider">消耗</div>
                            <div className="flex items-center gap-1.5">
                              <div className="text-[11px] font-mono font-bold text-amber-600">{formatCurrency(drama.spend, currency)}</div>
                              <div className={`text-[9px] font-medium ${drama.spendTrend > 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                                {drama.spendTrend > 0 ? '↑' : '↓'}{Math.abs(drama.spendTrend)}%
                              </div>
                            </div>
                          </div>
                          <div>
                            <div className="text-[9px] text-slate-400 font-medium uppercase tracking-wider">充值</div>
                            <div className="flex items-center gap-1.5">
                              <div className="text-[11px] font-mono font-bold text-teal-600">{formatCurrency(drama.revenue, currency)}</div>
                              <div className={`text-[9px] font-medium ${drama.trend > 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                                {drama.trend > 0 ? '↑' : '↓'}{Math.abs(drama.trend)}%
                              </div>
                            </div>
                          </div>
                          <div>
                            <div className="text-[9px] text-slate-400 font-medium uppercase tracking-wider">ROI</div>
                            <div className={`text-[11px] font-mono font-bold ${drama.roi >= 1.2 ? 'text-emerald-600' : 'text-slate-600'}`}>
                              {drama.roi.toFixed(2)}
                            </div>
                          </div>
                        </div>
        ...
        {/* Channel Rankings & Lead Ranking (Split Layout) */}
        <div className="grid grid-cols-12 gap-6 mt-6">
          {/* Left: Lead Ranking (优化师组长排行) */}
          <div className="col-span-12 lg:col-span-3 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
            <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-800">优化师组长排行</h3>
              </div>
            </div>
            <div className="p-4 space-y-4 overflow-auto max-h-[600px]">
              {leadRankings.map((lead, idx) => (
                <div key={lead.name} className="flex items-center gap-3 group">
                  <div className={`w-6 h-6 rounded flex items-center justify-center text-xs font-bold shrink-0 ${idx < 3 ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-100 text-slate-500'}`}>
                    {idx + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-slate-700 truncate group-hover:text-indigo-600 transition-colors">{lead.name}</span>
                        <span className="text-[9px] text-slate-400">{lead.department}</span>
                      </div>
                      <span className="text-xs font-mono font-bold text-indigo-600">{formatCurrency(lead.recharge, currency)}</span>
                    </div>
                    <div className="flex items-center justify-between text-[10px] text-slate-400">
                      <span>消耗: {formatCurrency(lead.spend, currency)}</span>
                      <span className={`font-bold ${lead.roi >= 1.3 ? 'text-emerald-500' : 'text-slate-500'}`}>ROI: {lead.roi.toFixed(2)}</span>
                    </div>
                    <div className="mt-1.5 w-full h-1 bg-slate-100 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-indigo-500 rounded-full" 
                        style={{ width: `${(lead.recharge / leadRankings[0].recharge) * 100}%` }}
                      />
                    </div>
                  </div>
                </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {channelData.map((item, idx) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right font-mono text-teal-600 font-semibold">{item.netRevenue ? formatCurrency(item.netRevenue, currency) : '-'}</td>
